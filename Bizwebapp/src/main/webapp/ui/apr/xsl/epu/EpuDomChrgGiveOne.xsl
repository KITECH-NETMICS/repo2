	...
	<xsl:template match="/">
		<xsl:variable name="localPath" select="request/localPath" />
			<BODY>
						...
						<td class="body">
							<table width="100%" border="0" cellpadding="0" cellspacing="0" class="aprinnertable">
								<tr>
									...
									<td class="contant">
										<a style="color: #0000FF;">
											<xsl:attribute name="href">javascript:javascript:openContrctPop('<xsl:value-of select="request/APPROVAL/contrct_no" />')</xsl:attribute>
								...
								<tr>
									...
									<td class="contant-center">
										<xsl:value-of select="substring(request/APPROVAL/suply_time_start_ymd, 1, 4)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/suply_time_start_ymd, 5, 2)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/suply_time_start_ymd, 7, 2)" />
										~
										<xsl:value-of select="substring(request/APPROVAL/suply_time_cls_ymd, 1, 4)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/suply_time_cls_ymd, 5, 2)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/suply_time_cls_ymd, 7, 2)" />
									</td>
									<td class='title2' width="16%">납품일</td>
									<td class="contant">
										<xsl:value-of select="substring(request/APPROVAL/delvr_ymd, 1, 4)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/delvr_ymd, 5, 2)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/delvr_ymd, 7, 2)" />
									</td>
									<td class='title2' width="16%">검수일</td>
									<td class="contant">
										<xsl:value-of select="substring(request/APPROVAL/check_ymd, 1, 4)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/check_ymd, 5, 2)" />
										-
										<xsl:value-of select="substring(request/APPROVAL/check_ymd, 7, 2)" />
									</td>
								</tr>
								<tr>
									...
									<td class="contant-right">
										<xsl:if test="request/APPROVAL/contrct_tot_amt != ''">
				...
				<xsl:if test="request/APPROVAL/attach_A_yn = 'Y'">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="aproutertable">
						<tr>
				...
				<xsl:if test="request/APPROVAL/attach_P_yn = 'Y'">
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="aproutertable">
						<tr>
							<td class="head"> 첨부파일</td>
						</tr>
						<tr>
							<td class="body">
								<table width="100%" border="0" cellpadding="0" cellspacing="0" class="aprinnertable">
									<xsl:for-each select="request/APPROVAL/fileList_P/vector/data/msgFbaExpnsEvidMS01">
										<tr>
											<td class='title2' width="16%"> <xsl:value-of select="position()" /></td>
											<td class='contant' width="84%">
												<a>
													<xsl:attribute name="href">javascript:downloadForXsl('<xsl:value-of select="attach_file_no" />01','XAA004')</xsl:attribute>
													<xsl:value-of select="evid_item_nm" />
												</a>
											</td>
										</tr>
									</xsl:for-each>
