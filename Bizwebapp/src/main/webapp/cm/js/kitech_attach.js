...
var KitechAttach = (function(Ws, Rn) {
	...
	var james = { // internal name of library
		 ...
		 * Constants Area
		 */
		  SERVLET: '/websquare/engine/proworks/callServletService.jsp'
		, BASE_TASK: 'kitech.com.service.CommonService'
		, BASE_ACTION: 'execute'
		
		/**
		 * Variable Area
		 */
		, uploadMap: new Ws.collection.Hashtable()
		, uploadObj: []
		, saveCallback: null
		...
		, setUploaded: function(uploadId, isSync){
			var dataMap = this.uploadMap.get(uploadId);
			dataMap.put('is_sync', isSync);
			this.uploadMap.put(uploadId, dataMap);
			debugger;
		}
		...
		, isUploaded: function(uploadId){
			var dataMap = this.uploadMap.get(uploadId);
			return Ws.util.getBoolean( dataMap.get('is_sync'));
		}
		...
		, isUploadedComplete: function(){
			return KitechAttach.uploadMap.key.every( function(uploadId){
		...
		, create: function(uploadId, obj){
			...
			var dataMap = new Ws.collection.Hashtable();
			Object.keys(obj).map(function (key) {
				dataMap.put(key, obj[key]);	
			});
			dataMap.put('is_sync', true);
			this.uploadMap.put(uploadId, dataMap);
			...
			// 로깅 출력
			// this.log( 'create(): 첨부파일 컴포넌트 생성 완료!!' +  Ws.xml.indent(this.uploadMap.toString()));
		}
		...
		, setData: function(uploadId, fileKey){
			...
			if (4 == fileKey.length){
				...
				KitechAttach.biz.setAttachFiles( uploadId, fileKey, dataMap.get('target'));
			}
		}
				...
				(this.saveCallback)(this.uploadMap.key.reduce( function(acc, uploadId){ 
					...
					if (dataArray.webFile){
						dataArray.webFile.some( function(data, index){
							acc[uploadId] = KitechAttach.util.makeAttachFileNo(data.uploadPath);
							return 0 === index;
						});
					}				 
					return acc;
...
//				});
		...
		, deleteAll: function(param){
			...
			if (arrayDel && 0 < arrayDel.length && arrayDel[0] != null){
				...
				arrayDel.forEach( function(del) {
					...
					arrayObj.push({attach_file_no: attachFileNo, seq: del.uniqKey, file_key: del.uploadPath, which: del.customValue});
				});
		...
		, getAttachFiles: function(uploadId){
			var dataArray = Rn.GetListInfo('array', uploadId);
			var vec = new Ws.collection.Vector();
			if (dataArray && dataArray.mergeFile){
				dataArray.mergeFile.forEach( function(data){
					var doc = Ws.xml.parse("<mdata/>");
					var fileName = data.originalName;
					Ws.xml.setString( doc, "attach_file_no", KitechAttach.util.makeAttachFileNo(data.uploadPath));
					Ws.xml.setString( doc, "seq", KitechAttach.util.makeAttachFileNoSeq(data.uploadPath));
					Ws.xml.setString( doc, "file_nm", fileName);
					Ws.xml.setString( doc, "extens_nm", fileName.substring( fileName.lastIndexOf('.')+1));
					Ws.xml.setString( doc, "size", data.size);
					vec.addElement(doc);
				});
			}
		...
		, event: {
			...
			creationComplate: function(uploadId){
				...
				var dataMap = KitechAttach.uploadMap.get(uploadId);
				KitechAttach.action.setUploadMode(dataMap.get('mode'), uploadId);

				var fileKey = dataMap.get('fileKey');
			...
			, afterAddFile: function(uploadId) {
				...
				KitechAttach.log('파일추가 완료(afterAddFile): ' + uploadId + ', attach_file_no 설정: ' + dataMap.get('fileKey'));
				Rn.AddFormData('attachFileNo', dataMap.get('fileKey'));
				// Rn.SetFileCustomValue("-1", dataMap.get('fileKey'),
				// uploadId);
			}
			...
			, beforeUpload: function(uploadId) {
				...
				var dataMap = KitechAttach.uploadMap.get(uploadId);
				Rn.AddFormData('target', dataMap.get('target'));
...
//						Rn.AddFormData('removeFiles', arrayDel.map(function(element){
...
//						}).join(','), uploadId);
		 ...
		 */					
		, biz: {
			  XDA_ATTACHS: 'kitech.com.xda.XomxComAttachFileMS01'	// 검증 액션

			, toXML: function(params, action, task){
				var doc = Ws.xml.parse('<param />');
				if (2 == arguments.length){
					Ws.xml.setAttribute(doc, 'task', KitechAttach.BASE_TASK);
					Ws.xml.setAttribute(doc, 'action', KitechAttach.BASE_ACTION);
					Ws.xml.setAttribute(doc, 'xda', action);
				} else if (3 == arguments.length){
					Ws.xml.setAttribute(doc, 'task', task);
					Ws.xml.setAttribute(doc, 'action', action);
				}
				if (params instanceof Array){
					// 삭제 시 다건 파라미터 처리를 위해
					var vec = new Ws.collection.Vector();
					params.forEach( function( param){
						var ele = Ws.xml.parse('<request/>');
						Object.keys(param).map(function (key) {
							Ws.xml.setString(ele, key, param[key]);	
						});
						vec.addElement(ele);
					});
					var vecDoc = Ws.xml.parse('<deleteVec/>'); 
					Ws.xml.appendChild( Ws.xml.findNode(vecDoc, 'deleteVec'), Ws.xml.findNode(vec.toDocument(), 'vector'));
					Ws.xml.appendChild( Ws.xml.findNode(doc, 'param'), Ws.xml.findNode(vecDoc, 'deleteVec'));
				} else {
					// 삭제 이외의 단건 파라미터
					Object.keys(params).map(function (key) {
						Ws.xml.setString(doc, key, params[key]);	
					});
				}
				return doc;
			}
			...
			, generateKey: function(uploadId, docId){
				var sysClsf = docId.substring(0,1);
				var docClsf = docId.substring(1,4);
				Ws.net.ajax({
					  action : KitechAttach.SERVLET
					, mode : 'synchronous'
					, mediatype : 'application/xml'
					, method : 'post'
					, requestData : Ws.xml.serialize(this.toXML(
							  {'SYSTEM_ID': sysClsf, 'DOCUMENT_ID': docClsf}
							, 'getGenerationFileNumber'
							, 'kitech.com.business.ComBiz'
						))
					, success : function(e){
						if (gfnSubmitMessageBar(e)) {
							var fileKey = Ws.xml.getString(e.responseBody, "GEN_FILENO_SEQ");
							...
							var dataMap = KitechAttach.uploadMap.get(uploadId);
							dataMap.put('fileKey', fileKey);
							KitechAttach.uploadMap.put(uploadId, dataMap);
			...
			, setAttachFiles: function(uploadId, fileKey, customValue){
				Ws.net.ajax({
					...
					, mediatype : 'application/xml'
					, method : 'post'
					, requestData : Ws.xml.serialize(this.toXML({'attach_file_no': fileKey}, 'kitech.com.xda.XomxComAttachFileMS01'))
					, success : function(e){
						var vec = Ws.collection.toVector(e.responseBody);
						for (var index = 0; index < vec.size(); index++){
							var doc = vec.elementAt(index);
							var fileUniqueKey = Ws.xml.getString( doc, "seq");
							var fileName = Ws.xml.getString( doc, "org_file");
							var filePath = '/' + Ws.xml.getString( doc, "attach_full_path");
							var fileSize = Ws.xml.getString( doc, "file_size");
							// 지재권 포털 서비스에서 업로드시 용량이 공통테이블에 바이트 단위로 들어가므로
							if (-1 < fileSize.indexOf('M')){
								fileSize = Ws.util.parseFloat( fileSize.replace(/[^0-9|.]/g, ''), 0) * 1024 * 1024;
							} else if (-1 < fileSize.indexOf('K')){
								fileSize = Ws.util.parseFloat( fileSize.replace(/[^0-9|.]/g, ''), 0) * 1024;
							} else {
								fileSize = Ws.util.parseFloat( fileSize, 0);
							}
							// console.log(fileUniqueKey, fileName, filePath, fileSize, customValue, uploadId);
			...
			, deleteFiles: function(params, removeCallback){
				Ws.net.ajax({
					...
					, mediatype : 'application/xml'
					, method : 'post'
					, requestData : Ws.xml.serialize(this.toXML(params, 'deleteXomxComAttachWithFile', 'kitech.com.business.ComBiz'))
					, success : function(e){
						if (gfnSubmitMessageBar(e)) {
							console.log('첨부파일 삭제 완료!!', e.responeBody);
							if (removeCallback){
								console.log('첨부파일 삭제 후 콜백 수행!!', e.responeBody);
								(removeCallback)();
							}
						}
					}
