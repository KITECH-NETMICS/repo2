...
/**
 * BPM 결재리스트를 다건 조회 API 연계 클라이언트 구현체. (인스턴스단위: 각 완료함)
 *
 * @author James
 * @since 2022. 5. 20.
 */
@Component("bpmGetRunWorks")
@Scope(value = "prototype", proxyMode = ScopedProxyMode.TARGET_CLASS)
public class BpmGetRunWorksClient implements BpmClient {
	public static String SERVICE = "/rest/bpm/process-instances/{status}/";
	...
	public String makeEndPoint(BpmRequest req){
    	this.proc = (StringUtils.isEmpty(req.getProc())) ? "list" : req.getProc();

		HttpUrl template = HttpUrl.parse(this.baseUrl + SERVICE + this.proc);
		HttpUrl.Builder builder = template.newBuilder();
		if (StringUtils.isNotEmpty(req.getStatus())) {
			builder.setPathSegment( ArrayUtils.indexOf(template.pathSegments().toArray(), "{status}"), req.getStatus());
		}
		
		if (StringUtils.isNotEmpty(req.getEndpoint())) builder.addQueryParameter("endpoint",req.getEndpoint());
		...
		if (StringUtils.isNotEmpty(req.getPrevEndpoint())) builder.addQueryParameter("prevEndpoint", req.getPrevEndpoint());
		...
		// 모든 사용자 전체 조회인 경우 alias 필터가 필수
		if (StringUtils.isNotEmpty(req.getAlias()) && "system".equals(req.getEndpoint())) {
			builder.addQueryParameter("alias", req.getAlias());
		}
