 ...
 * @since 2022. 5. 3.
 */
@Component("bpmGetInst")
@Scope(value = "prototype", proxyMode = ScopedProxyMode.TARGET_CLASS)
public class BpmGetInstClient implements BpmClient {
	public static String GET_SERVICE = "/rest/bpm/process-instances/{alias}/get";
	...
	public String makeEndPoint(BpmRequest req){
		...
		builder.setPathSegment( ArrayUtils.indexOf(template.pathSegments().toArray(), "{alias}"), req.getAlias());
		
		builder.addQueryParameter("endpoint", req.getEndpoint());
		if (StringUtils.isNotEmpty(req.getInstanceId())) builder.addQueryParameter("instanceId", req.getInstanceId());
		if (StringUtils.isNotEmpty(req.getTaskId())) builder.addQueryParameter("taskId", req.getTaskId());
		if (StringUtils.isNotEmpty(req.getTrcTag())) builder.addQueryParameter("trcTag", req.getTrcTag());
	...
	public BpmResult parseResponse(JSONObject jobject) {
		try {
			ApiResult apiResult = new ObjectMapper().readValue(jobject.get(RES_COL_NAME_DATA).toString(), ApiGetResult.class);
